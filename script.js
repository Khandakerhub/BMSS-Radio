document.addEventListener('DOMContentLoaded', function () {
	...
	const stationNodeList = document.querySelectorAll('.station'); // NodeList of stations
	const stationElements = Array.from(stationNodeList); // Convert to Array for easier index finding
	...
	masterBtn.addEventListener('click', () => {
		if (currentRadio) {
			...
			playStation(stationElements[0]); // Play first station if none loaded
		}
	});

	// --- Backward Button Control ---
	backwardBtn.addEventListener('click', () => {
		if (stationElements.length === 0) return; // No stations to navigate

		let currentIndex = -1;
		if (currentStationElement) {
			currentIndex = stationElements.indexOf(currentStationElement);
		}

		let newIndex;
		if (currentIndex <= 0) { // If no station playing or first station playing
			newIndex = stationElements.length - 1; // Go to the last station
		} else {
			newIndex = currentIndex - 1; // Go to previous station
		}
		playStation(stationElements[newIndex]);
	});
	...
	forwardBtn.addEventListener('click', () => {
		...
		let newIndex;
		if (currentIndex === -1 || currentIndex === stationElements.length - 1) { // If no station playing or last station playing
			newIndex = 0; // Go to the first station
		} else {
			newIndex = currentIndex + 1; // Go to next station
		}
		playStation(stationElements[newIndex]);
	});


	// --- Station Controls (Clicking on a station in the list) ---
	stationElements.forEach(stationEl => {
		const btn = stationEl.querySelector('.station-play-btn');
		stationEl.addEventListener('click', (e) => {
			// If click is on the station's own play/pause button and it's the current station
			if (e.target.closest('.station-play-btn') && currentStationElement === stationEl) {
				...
				return; // Handled by this specific button
			}
			// If station is different from current, play it
			if (currentStationElement !== stationEl) {
				playStation(stationEl);
			} else if (currentRadio && !currentRadio.playing()) { // If same station but paused, play it
				currentRadio.play();
			}
		});

		// Event listener for the play/pause button within each station item
		btn.addEventListener('click', (e) => {
			e.stopPropagation(); // Prevent the stationEl click handler from firing as well
			if (currentStationElement !== stationEl) { // If it's a new station
				playStation(stationEl);
			} else { // If it's the current station, toggle play/pause
				if (currentRadio.playing()) {
					currentRadio.pause();
				} else {
					currentRadio.play();
	...
	function playStation(stationEl) {
		if (!stationEl) { // Safety check if a null stationEl is passed
			console.warn("playStation called with null station element.");
			return;
		}
		...
		currentRadio = new Howl({
			...
			onstop: function () {
				...
				updateMasterPlayPauseIcons(false);
				updateStationPlayPauseIcons(stationEl, false); // Ensure the stopped station's icon is reset
				if (canvasContext) {
					canvasContext.clearRect(0, 0, visualizer.width, visualizer.height);
				}
				// Only reset metadata if the stopped radio is the one we thought was current
				// And it's not being immediately replaced by a new playStation call (which would set currentRadio to new Howl)
	...
	function updateStationPlayPauseIcons(stationEl, isPlaying) {
		...
		// Reset icons for all other stations to ensure only one shows as playing (or its specific state)
